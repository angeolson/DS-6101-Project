...
To vusialize the average price along side density, we used the `ggpmap` package to build the map shown below. The size of the point indicates the density of the neighborhood and the color indicates the average price. 

As we mentioned before, Manhattan is the most densely populated with Airbnbs. The figure below shows that it is also the most expensive. Likewise, we mentioned Staten Island is the least densely populated, and we can see that it is also the least expensive. 
```{r}
