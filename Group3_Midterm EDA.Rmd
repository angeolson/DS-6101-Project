...
Before we began analysis, we took a few steps to clean the data. The data was mostly cleaned, but we created a subset for calculating correlation, changed some data to be categorical or numerical depending on the variable, and fixed the dates so that they aren't read in as characters.
...
As table above shows a general summary of the statistics behind our dataset. A few things to take away is that the price ranges from 0 USD to 10,000 USD. People stayed spent anywhere from 1 to 1,250 nights in an Airbnb. At the end of the year, each Airbnb had reviews ranges from 0 to 629. Finally hosts listed Airbnb's anywhere from 0 to 327 units. On average Airbnbs in New York City cost 142 USD, and on average people leave reviews once a month.
...
Correlation was used to test for relationships between `price` and the continuous variables in our dataset, and ANOVA tests were used to test for differences in the value of `price` between our categorical variables. Where our ANOVA tests showed statistically significant differences, we ran Tukey HSD Post-Hoc tets to determine where these differences occurred.
...
There appear to be a large number of outliers in the data and if we remove these outliers, 2972 observations are affected, the average price across New York city comes down to 120 USD per night, and the maximum price per night has changed to 334 USD. 
...
We can tell the density of AirBnB's distribution by looking at the graphs, and we can see that Manhattan is the most dense and Staten Island is the least dense.
...
To sum up the average price and density, we used the `ggpmap` package to build this map, where the size of the point indicates the density of the neighborhood and the color indicates the average price. 

This map depicts how densely Airbnbs are located in Manhattan, as well as the average price charged, as indicated by the large circles.Staten Island is the least densely populated, with small circles signifying a cheap price. 
```{r}
