...
xkablesummary(airbnb_cor)
library(DescTools)
for (i in 1:ncol(airbnb_cor))
  {mod_val <- Mode(airbnb_cor[,i])
  cat(i, ": ",mod_val,"\n")}
sd(airbnb_cor$price)
sd(airbnb_cor$minimum_nights)
sd(airbnb_cor$number_of_reviews)
sd(airbnb_cor$reviews_per_month)
sd(airbnb_cor$calculated_host_listings_count)
sd(airbnb_cor$availability_365)
```
...
ggplot(data=airbnb, aes(price)) + 
  ...
  labs(title="Histogram of AirBnB Price") 
```

## qqplot for Price (All Observations)
```{r, results='markup'}
qqnorm(airbnb$price, pch = 20, main = "Q-Q Plot for All AirBnB Prices")
qqline(airbnb$price, col = "yellow", lwd = 2)
```
...
```{r, results='markup'}
qqnorm(airbnb_clean$price, pch = 20, main = "Q-Q Plot for All AirBnB Prices")
qqline(airbnb_clean$price, col = "yellow", lwd = 2)
```
