...
library(ggmap)
library(tmaptools)
ggmap(get_stamenmap(rbind(as.numeric(paste(geocode_OSM("New York")$bbox))), zoom = 10)) + geom_point(data = airbnb, alpha = .01, aes(x = longitude, y = latitude))
...
newyork.map <- ggmap(get_map(location= 'Lower Manhattan, New York', 
  ...
  geom_point(data=airbnb, aes(x=longitude,y=latitude), color='red',size=2)
newyork.map
```
